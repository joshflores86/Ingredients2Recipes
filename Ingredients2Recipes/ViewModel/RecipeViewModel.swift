...
class RecipeViewModel: ObservableObject {
    ...
    var ingredients: [OnlineIngredientModel] = []
//    private var name: String = ""
    @Published var customRecipes: [RecipeModel] = []
    @Published var onlineRecipes: [OnlineRecipeModel] = []
    @Published var inHouseRecipe: [RecipeModel] = [
        RecipeModel(name: "Pancakes",
                    ingredient: [
                        ...
                        IngredientModel(ingredients: "Salt", unit: .teaSpoon, quantity: Int64(0.5)),
        ...
        RecipeModel(name: "Fluffy Pancakes",
                    ingredient: [
                        ...
                        IngredientModel(ingredients: "Milk", unit: .cup, quantity: 1),
                        IngredientModel(ingredients: "Unsalted butter, melted, or vegetable oil", unit: .tableSpoon, quantity: 2),
                        IngredientModel(ingredients: "Egg", unit: .qty, quantity: 1),
    ...
    func saveOnlineRecipe(_ recipe: OnlineRecipeModel) {
        let managedObjectContext = coreDataStack.viewContext
        ...
        newRecipe.name = recipe.name
        newRecipe.instructions = recipe.instructions
        newRecipe.hours = Int16(recipe.timeHours)
        newRecipe.minute = Int16(recipe.timeMinute)
        newRecipe.favorite = recipe.favorite
        ...
        for ingredient in recipe.ingredient {
            ...
            ingredientEntity.ingredient = ingredient.ingredients
            ingredientEntity.unit = ingredient.unit.rawValue
            ingredientEntity.quantity = Int64(ingredient.quantity)
            ingredientsSet.add(ingredientEntity)
        }
        newRecipe.ingredients = ingredientsSet
        coreDataStack.saveContext()
    ...
    func getData(completion: @escaping () -> Void) {
        ...
        db.collection("recipes").getDocuments {[weak self] snapshot, error in
            ...
            if let error = error {
                ...
                self.onlineRecipes = snapshot?.documents.compactMap({ document in
                    try? document.data(as: OnlineRecipeModel.self)
                }) ?? []
                completion()
            }
...
//                print("\(document.documentID) => \(document.data())")
                ...
                let instruction = document["instructions"] as? String ?? ""
                let ingredientData = document["ingredient"] as? [[String:Any]] ?? []
                let hours = document["timeHours"] as? Int ?? 0
                let minute = document["timeMinute"] as? Int ?? 0
                let favorite = document["favorite"] as? Bool ?? false
                
                
                for documents in ingredientData {
                    let name = documents["name"] as? String ?? ""
                    let unitString = documents["unit"] as?  String ?? ""
                    let quantity = documents["quantity"] as? Int ?? 0
                    let unit = OnlineIngredientModel.UnitOfMeasure(rawValue: unitString) ?? .none
                    let ingredient = OnlineIngredientModel(ingredients: name, unit: unit, quantity: quantity)
                    ...
                    let recipe = OnlineRecipeModel(name: name,
                                             ...
                                             timeMinute: minute,
                                             favorite: favorite)
                print(recipe.timeHours)
                    onlineRecipes.append(recipe)
            }
            completion()
        }
